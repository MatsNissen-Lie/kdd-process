...
\title{{\myfont Data Science Project by Mats}}  % Title
...
\subsection*{\textit{Variables Encoding}}
  % Symbolic Variables: Applied OneHotEncoder for PD_DESC, OFNS_DESC, LAW_CODE, and ARREST_BORO to handle categorical data with many unique values, creating dummy variables. 
  % Age Group: Mapped categories ('18-24', '25-44', etc.) to ordinal integers. Missing or unknown values assigned -1.
  % Race: Encoded PERP_RACE categories as integers for simplicity and compatibility with modeling.
  % Cyclic Variables: Not present in this dataset.
  % write this as text in latex

%   % write the text in latex
% \text{
%   \textbf{Symbolic Variables:} Applied OneHotEncoder for PD\_DESC, OFNS\_DESC, LAW\_CODE, and ARREST\_BORO to handle categorical data with many unique values, creating dummy variables.\\
%   \textbf{Age Group:} Mapped categories ('18-24', '25-44', etc.) to ordinal integers. Missing or unknown values assigned -1.
%   \text{Race:} Encoded PER_RACE categories as integers for simplicity and compatibility with modeling.
%   \textbf{Cyclic Variables:} Not present in this dataset.
% }
...
\ctext[RGB]{190,190,190}{Comparing no handling, dropping, fixed-value replacement, and truncation: Dropping outliers yielded the highest accuracy (0.99) but cut the dataset size drastically (3672→632). Truncation and no handling maintained data integrity but lowered accuracy. Balancing performance and data representativeness, doing nothing is chosen. }
\begin{figure}[H]
%\centering\includegraphics[scale=0.95]{}
